...
variable "shape" {
  description = <<-EOT
    Compute shape for the db system's instance node. PostgreSQL.VM.Standard.E5.Flex is
...
variable "instance_ocpu_count" {
  description = "OCPU count per db system instance node (PostgreSQL.VM.Standard.E5.Flex supports 1-64)."
  type        = number
  default     = 2
}

variable "instance_memory_size_in_gbs" {
  description = "Memory, in GB, per db system instance node (PostgreSQL.VM.Standard.E5.Flex supports 16-1024, default 16/OCPU)."
  type        = number
  default     = 32
}
...
variable "admin_password_secret_version" {
  description = <<-EOT
    Secret version of the Vault secret holding the admin password. Passed as
    credentials.password_details.secret_version (required when
    password_type = VAULT_SECRET). Required input — no default.
  EOT
  type        = string

  validation {
    condition     = try(trimspace(var.admin_password_secret_version), "") != ""
    error_message = "admin_password_secret_version is required and cannot be empty or whitespace-only."
  }
}
